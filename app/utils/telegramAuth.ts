...
export function validateTelegramWebAppData(telegramInitData:string):ValidationResult{
    ...
    if(!BOT_TOKEN){
        return {
            message: 'BOT_TOKEN is no set', validatedData: null, user: {}
        }
    }
    ...
    if(!authDate){
        return {
            message: 'auth_date is missing from initData', validatedData: null, user: {}
        }
    }
    ...
    if(timeDifference > fiveMinutesInSeconds){
        return {
            message: 'Telegram data is older than 5 minutes', validatedData: null, user: {}
        }
    }
