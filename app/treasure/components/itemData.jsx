...
export default function ItemData({ item,setItem}) {
  return (
    <div className="flex flex-col items-center p-4 bg-gray-700 border border-gray-600 rounded-md" onDoubleClick={() => setItem(null)}>
      ...
      <span className="text-lg">Price: <span className="text-yellow-400">{item ? item.price : 0}</span></span>
        <span className="text-lg">Lvl for item: <span className="text-yellow-400">{item ? item.lvlBosses : 0}</span></span>
        <span className="text-lg">Damage: <span className="text-yellow-400">{item ? item.damage : 0}</span></span>
      </div>
